...
<div class="py-6">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6">
                <h2 class="text-2xl font-bold text-gray-800 mb-6">Modifier la séance</h2>
                ...
                <form action="{{ route('seances.update', $seance) }}" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="date" name="date_de_seance" id="date_de_seance" 
                            ...
                            value="{{ old('date_de_seance', $seance->date_de_seance) }}" required>
                        ...
                        <select name="niveau_id" id="niveau_id" 
                            ...
                            @foreach($niveaux as $niveau)
                                <option value="{{ $niveau->id }}" 
                                    {{ (old('niveau_id', $seance->niveau_id) == $niveau->id) ? 'selected' : '' }}>
                                    {{ $niveau->nom_niveau }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <select name="id_groupe" id="id_groupe" 
                            ...
                            @foreach($groupes as $groupe)
                                <option value="{{ $groupe->id_groupe }}" 
                                    {{ (old('id_groupe', $seance->id_groupe) == $groupe->id_groupe) ? 'selected' : '' }}>
                                    {{ $groupe->nom_groupe }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <select name="id_module" id="id_module" 
                            ...
                            @foreach($modules as $module)
                                <option value="{{ $module->id_module }}" 
                                    {{ (old('id_module', $seance->id_module) == $module->id_module) ? 'selected' : '' }}>
                                    {{ $module->nom_module }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <select name="id_user" id="id_user" 
                            ...
                            @foreach($users as $user)
                                <option value="{{ $user->id_user }}" 
                                    {{ (old('id_user', $seance->id_user) == $user->id_user) ? 'selected' : '' }}>
                                    {{ $user->name }}
                                </option>
                            @endforeach
                        </select>
