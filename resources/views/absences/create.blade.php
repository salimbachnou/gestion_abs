...
<div class="py-6">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6">
                <h2 class="text-2xl font-bold text-gray-800 mb-6">Ajouter une absence</h2>
                ...
                <form action="{{ route('absences.store') }}" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <select name="id_seance" id="id_seance" 
                            ...
                            @foreach($seances as $seance)
                                <option value="{{ $seance->id }}" {{ old('id_seance') == $seance->id ? 'selected' : '' }}>
                    ...
                    <div class="mb-4">
                        ...
                        <select name="justifiee" id="justifiee" 
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                            <option value="0" {{ old('justifiee') == '0' ? 'selected' : '' }}>Non</option>
                            <option value="1" {{ old('justifiee') == '1' ? 'selected' : '' }}>Oui</option>
                        </select>
                        @error('justifiee')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
